...
function unlockNextStep() {
    ...
    // Unlock the content block visually
    const nextStepContent = document.getElementById(`step-${appState.maxUnlockedStep}`);
    if (nextStepContent) {
        nextStepContent.classList.add('unlocked');
        // Optional: Scroll to it automatically? The user said "you scroll and run", implying manual scrolling or seamless flow.
        // Let's not auto-scroll unless requested, validation showed user wants to "scroll and run".
        // But maybe we should make it 'active' so it pops?
...
stepButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (stepNum > appState.maxUnlockedStep && !appState.completedSteps.includes(stepNum)) {
            // Allow viewing if it's the next available step or already unlocked
            if (stepNum !== appState.maxUnlockedStep) {
        ...
        // Highlight this step, unhighlight others (but don't hide them)
    ...
    runButtons.forEach((btn) => {
        ...
        btn.addEventListener('click', function () {
            ...
            setTimeout(() => {
                ...
                this.disabled = false; // Allow re-running if needed, or keep disabled if preferred. Reference enables it.
            }, 1000);
        });
    });
}
