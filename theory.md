...
&Phi;(x) = W<sub>n</sub>ρ · W<sub>n-1</sub> · ... · ρ · W<sub>1</sub>(x)
...
Up till now, neural networks where the output from one layer is used as input to the next layer. Such networks are called **feedforward neural networks**. This means there are no loops in the network - information is always feed forward, never feedback which can also be shown as in Figure 1.
